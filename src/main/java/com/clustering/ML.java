...
public class ML {
    ...
    public double getCentroid(double[] data) {
        ...
        for(int i = 0; i < data.length; ++i) {
            initCentroid = initCentroid + data[i];
        }
        double centroid = initCentroid / (double) data.length;
        return centroid;
    }

    public double getCentroid(int[] data) {
        double initCentroid = 0.0;
        for(int i = 0; i < data.length; ++i) {
            initCentroid = initCentroid + (double) data[i];
        }
        double centroid = initCentroid / (double)data.length;
        return centroid;
    }
    ...
    public double[][] getCentroid(double[][] data, int[] clusters) {
        int[] mm = new com.bayudwiyansatria.utils.Utils().getUnique(clusters);
        double[][] centroid = new double[mm.length][data[0].length];

        for(int i = 0; i < mm.length; ++i) {
            int[] pos = new com.bayudwiyansatria.utils.Utils().getFind(clusters, "=", mm[i]);
            double[] initCentroid = new com.bayudwiyansatria.mat.Mat().initArray(data[0].length, 0.0);

            for(int j = 0; j < pos.length; ++j) {
                for(int d = 0; d < data[0].length; ++d) {
                    initCentroid[d] += data[pos[j]][d];
                }
            }

            centroid[i] = new com.bayudwiyansatria.math.Math().Calculate(initCentroid, (double)pos.length, "/");
        }

        return centroid;
    }

    public double[][] getCentroid(int[][] data, int[] clusters) {
        int[] mm = new com.bayudwiyansatria.utils.Utils().getUnique(clusters);
        double[][] centroid = new double[mm.length][data[0].length];
        ...
        for(int i = 0; i < mm.length; ++i) {
            ...
            for(int j = 0; j < pos.length; ++j) {
                for(int d = 0; d < data[0].length; ++d) {
                    initCentroid[d] += (double)data[pos[j]][d];
                }
            }
